...
一个安装单元，把施工纪律、真实验收、资料演进、代码定位和只读进化史连接起来。当前版本 **v0.9.0**；完整包平台范围仍为 **macOS arm64**，Codex 与 Cursor 共用同一份技能与 CLI。

## 根目录阅读

授权施工时技能自动建立 `东合项目进化史.html`，沿用项目已有资料体系。双击可看离线快照；点“连接实时资料”唤起安装包自带阅读助手，再在页面刷新资料或重建索引，不需要手动启动服务。任务线下面展示任务流水，原文和历史资料可搜索展开。阅读接入不要求采用 CLI 管理任务，也不执行 init/迁移。

安装包一次注册 `donghe://` 协议，浏览器首次可能要求确认打开助手。助手闲置 15 分钟退出，无登录常驻服务；刷新读取已经落盘的资料，不依赖施工完工。详见[项目阅读入口](playbooks/13-项目阅读入口.md)。
...
上述施工策略沿用 v0.8.2；v0.9.0 新增 reader 接入、刷新、打开与助手安装命令。
